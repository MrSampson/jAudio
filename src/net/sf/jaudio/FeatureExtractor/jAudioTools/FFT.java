 ...
 *
 * <p>
 * The FFT is performed by this class' constructor. The real and imaginary
...
public class FFT {
     ...
     *
     * @param real_input
     *            The real part of the signal to be transformed.
     * @param imaginary_input
     *            The imaginary part of the signal to be. transformed. This may
     *            be null if the signal is entirely real.
     * @param inverse_transform
     *            A value of false implies that a forward transform is to be
     *            applied, and a value of true means that an inverse transform
     *            is tob be applied.
     * @param use_hanning_window
     *            A value of true means that a Hanning window will be applied to
     *            the real_input. A value of valse will result in the
    ...
    public FFT(double[] real_input, double[] imaginary_input,
        ...
        if (imaginary_input != null)
            if (real_input.length != imaginary_input.length)
                throw new Exception(
                        "Imaginary and real inputs are of different sizes.");

        // Throw an exception if less than three samples are provided
        if (real_input.length < 3)
            throw new Exception("Only " + real_input.length
        ...
        int valid_size = net.sf.jaudio.FeatureExtractor.GeneralTools.Statistics
                .ensureIsPowerOfN(real_input.length, 2);
        ...
        if (use_hanning_window) {
            for (int i = 0; i < real_output.length; i++) {
                double hanning = 0.5 - 0.5 * Math.cos(2 * Math.PI * i
                        / valid_size);
        ...
        for (max_spectra_for_stage = 1, step_size = 2 * max_spectra_for_stage; max_spectra_for_stage < valid_size; max_spectra_for_stage = step_size, step_size = 2 * max_spectra_for_stage) {
            double delta_angle = forward_transform * Math.PI
                    / max_spectra_for_stage;
            ...
            for (int spectra_count = 0; spectra_count < max_spectra_for_stage; ++spectra_count) {
                ...
                for (int left = spectra_count; left < valid_size; left += step_size) {
                    ...
                    double temp_imag = real_correction
                            * imaginary_output[right] + imag_correction
                            * real_output[right];
                    real_output[right] = real_output[left] - temp_real;
                    imaginary_output[right] = imaginary_output[left]
                            - temp_imag;
     ...
     *
     * <p>
     * Only the left side of the spectrum is returned, as the folded portion of
     * the spectrum is redundant for the purpose of the magnitude spectrum. This
    ...
    public double[] getMagnitudeSpectrum() {
        ...
        if (output_magnitude == null) {
            ...
            for (int i = 0; i < output_magnitude.length; i++)
                output_magnitude[i] = (Math.sqrt(real_output[i]
                        * real_output[i] + imaginary_output[i]
                        * imaginary_output[i]))
                        / real_output.length;
        }
     ...
     * <p>
     * Only the left side of the spectrum is returned, as the folded portion of
     * the spectrum is redundant for the purpose of the power spectrum. This
     * means that the bins only go up to half of the sampling rate.
     *
     * @return The magnitude of each frequency bin.
     */
    public double[] getPowerSpectrum() {
        ...
        if (output_power == null) {
            ...
            for (int i = 0; i < output_power.length; i++)
                output_power[i] = (real_output[i] * real_output[i] + imaginary_output[i]
                        * imaginary_output[i])
                        / real_output.length;
        }
     ...
     * <p>
     * Only the left side of the spectrum is returned, as the folded portion of
     * the spectrum is redundant for the purpose of the phase angles. This means
     * that the bins only go up to half of the sampling rate.
     *
        ...
        if (output_angle == null) {
            ...
            for (int i = 0; i < output_angle.length; i++) {
                if (imaginary_output[i] == 0.0 && real_output[i] == 0.0)
                    ...
                    output_angle[i] = Math.atan(imaginary_output[i]
                            / real_output[i])
                            * 180.0 / Math.PI;
     ...
     *
     * @param sampling_rate
     *            The sampling rate that was used to perform the FFT.
