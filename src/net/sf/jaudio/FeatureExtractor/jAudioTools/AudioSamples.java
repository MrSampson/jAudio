...
public class AudioSamples {
    ...
    public AudioSamples(double[][] audio_samples, AudioFormat audio_format,
        ...
        if (audio_samples.length == 1)
            ...
            this.channel_samples = DSPMethods.getCopyOfSamples(audio_samples);
    ...
    public AudioSamples(double[][] audio_samples, float sampling_rate,
        ...
        this.audio_format = getDefaultAudioFormat(sampling_rate);
    ...
    public AudioInputStream getAudioInputStreamMixedDown() throws Exception {
        ...
        AudioFormat mixed_down_audio_format = new AudioFormat(
                this.audio_format.getSampleRate(),
                this.audio_format.getSampleSizeInBits(), 1, true,
                this.audio_format.isBigEndian());

        // Convert samples to 2-D array
        double[][] samples_to_convert = new double[1][];
        samples_to_convert[0] = this.samples;
    ...
    public double[][] getSamplesChannelSegregated() {
        if (this.channel_samples == null) {
            ...
            formatted_samples[0] = this.samples;
    ...
    public double[][][] getSampleWindowsChannelSegregated(int window_size)
        ...
        for (int chan = 0; chan < this.channel_samples.length; chan++)
            for (int win = 0; win < number_windows; win++) {
                if (win != number_windows - 1) {
                    for (int samp = 0; samp < window_size; samp++)
                        windowed_samples[chan][win][samp] = this.channel_samples[chan][win
                                * window_size + samp];
                } else {
                    for (int samp = 0; samp < window_size; samp++) {
                        if (win * window_size + samp < this.samples.length)
    ...
    public double checkMixedDownSamplesForClipping() {
        ...
        for (int samp = 0; samp < this.samples.length; samp++)
            if (Math.abs(this.samples[samp]) > 1.0) {
                double difference = Math.abs(this.samples[samp]) - 1.0;
    ...
    public double checkChannelSegregatedSamplesForClipping() {
        ...
        if (this.channel_samples != null) {
            for (int chan = 0; chan < this.channel_samples.length; chan++)
                for (int samp = 0; samp < this.channel_samples[chan].length; samp++)
                    if (Math.abs(this.channel_samples[chan][samp]) > 1.0) {
                        double difference = Math
                                .abs(this.channel_samples[chan][samp]) - 1.0;
                        if (difference > max_difference)
                            max_difference = difference;
                    }
        } else
    ...
    private int convertTimeToSampleIndex(double time) {
        int sample_index = (int) (time * this.audio_format.getSampleRate());
        if (sample_index < 0)
            return 0;
        else if (sample_index >= this.samples.length)
