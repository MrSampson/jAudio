...
import net.sf.jaudio.FeatureExtractor.jAudioTools.*;
...
public class RecordingSelectorPanel extends JPanel implements ActionListener {
	...
	private void viewRecordingInformation() {
		...
		for (int i = 0; i < selected_rows.length; i++) {
			try {
				...
				String data = net.sf.jaudio.FeatureExtractor.jAudioTools.AudioMethods
	...
	private String chooseSavePath() {
		...
		// chosen
		{
			...
			String ext = net.sf.jaudio.FeatureExtractor.GeneralTools.StringMethods
					.getExtension(path);
			if (ext == null) {
				path += ".xml";
				to_save_to = new File(path);
			} else if (!ext.equals(".xml")) {
				path = net.sf.jaudio.FeatureExtractor.GeneralTools.StringMethods
